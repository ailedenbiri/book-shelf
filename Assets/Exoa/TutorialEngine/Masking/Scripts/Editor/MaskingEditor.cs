...
namespace Maskable.Editor
{
	...
	public class MaskingEditor : UnityEditor.Editor
	{
		...
		public override void OnInspectorGUI()
		{
			...
			{
				switch ((Masking.MaskSource)_source.enumValueIndex)
				{
					case Masking.MaskSource.Graphic:
						break;
					case Masking.MaskSource.Sprite:
						...
						if ((Masking.BorderMode)_spriteBorderMode.enumValueIndex != Masking.BorderMode.Simple)
							EditorGUILayout.PropertyField(_spritePixelsPerUnitMultiplier, Labels.SpritePixelsPerUnitMultiplier);
						break;
					case Masking.MaskSource.Texture:
		...
		void ShowErrorsIfAny()
		{
			...
			ShowErrorIfPresent(errors, Masking.Errors.UnsupportedShaders, Labels.UnsupportedShaders, MessageType.Warning);
			ShowErrorIfPresent(errors, Masking.Errors.NestedMasks, Labels.NestedMasks, MessageType.Warning);
			ShowErrorIfPresent(errors, Masking.Errors.TightPackedSprite, Labels.TightPackedSprite, MessageType.Error);
			ShowErrorIfPresent(errors, Masking.Errors.AlphaSplitSprite, Labels.AlphaSplitSprite, MessageType.Error);
			ShowErrorIfPresent(errors, Masking.Errors.UnsupportedImageType, Labels.UnsupportedImageType, MessageType.Error);
			ShowErrorIfPresent(errors, Masking.Errors.UnreadableTexture, Labels.UnreadableTexture, MessageType.Error);
			ShowErrorIfPresent(errors, Masking.Errors.UnreadableRenderTexture, Labels.UnreadableRenderTexture, MessageType.Error);
		}

		Masking.Errors CollectErrors()
		{
			var result = Masking.Errors.NoError;
			foreach (var t in targets)
				result |= ((Masking)t).PollErrors();
			return result;
		}

		static void ShowErrorIfPresent(
				Masking.Errors actualErrors,
				Masking.Errors expectedError,
				string errorMessage,
